...
function ProjectCard({ image, alt, name, id }) {
  ...
  return (
    <div
      className="flex flex-col w-3/12 max-md:w-full transition-transform duration-300 hover:-translate-y-4"
      onClick={() => navigate(`project/${id}`)}
    >
      <div className="relative grow shrink-0 mt-9 mx-auto shadow-lg backdrop-blur-sm aspect-[0.83] w-[295px] max-md:mt-10">
        <img
          loading="lazy"
          src={image}
          alt={alt}
        ...
        />
        <div className="absolute bottom-0 w-full py-2 bg-opacity-100 text-white font-semibold text-center">
          {name}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col justify-center bg-white">
      <section className="flex flex-col items-center p-20 w-full bg-neutral-800 max-md:px-5 max-md:max-w-full">
        ...
        <div className="mt-9 w-full max-w-[1240px] max-md:w-full mx-auto">
          <div className="flex gap-5 justify-center max-md:flex-col max-md:gap-5">
